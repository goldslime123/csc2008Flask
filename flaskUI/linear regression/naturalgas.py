...
# import data set
datasetTemp = pd.read_csv('../csv/products/naturalgas.csv')
...
print("Predicted Natural Gas: 2022 Quarter 1: " + str(y))
...
plt.title('Natural Gas vs Tariff (Training set)')
...
plt.plot(X_trainTariff, regressorTariff.predict(X_trainTariff), color='blue')
plt.title('Natural Gas vs Tariff (Test set)')
plt.xlabel('Natural Gas')
plt.ylabel('Tariff')
plt.show()
